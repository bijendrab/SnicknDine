...
public class UserRestaurantBindController {
    ...
    @GetMapping("/userBind")
    public ResponseEntity<?> getUserBindRestaurant() {
        Map<String, Object> response = new HashMap<String, Object>();
        UserRestBindOutput userRestBindOutput= userRestBindService.getUserBindRestaurant();
        response.put("message","get Active Restaurant for the user");
        response.put("body",userRestBindOutput);
        response.put("error", Boolean.FALSE);
        response.put("status", HttpStatus.OK);
        return new ResponseEntity<>(response, HttpStatus.OK);
